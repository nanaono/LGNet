...
from Network.Proposed_v1 import BGSNet_AOTGAN
from Network.AOTGAN_network import Discriminator
...
if __name__ == '__main__':
    ...
    ### Train DATA path
    input_path = '/VCM_SourceCode/dataset/training/synthesis/*.png'
    mask_path = '/VCM_SourceCode/dataset/training/mask/*.png'
    gt_path = '/VCM_SourceCode/dataset/training/gt/*.png'
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                ...
                for i, data in enumerate(data_train_loader, 0):
                    ...
                    pixel_loss = L1_loss(fake_img * inv_mask, gt * inv_mask)
                    ...
                    tot_loss =  0.01*g_loss + pixel_loss + 0.1*lpips_loss_ + 250*style_loss_ 
                    ...
                    if is_main_process():
                        if count % 50 == 0:
                            ...
                            print('-----Generator-----')
                            print('Pixel loss: %.4f \t G loss: %.4f  \t Perc. loss: %.4f \t Style loss: %.4f  ' %(
                                    pixel_loss, g_loss_, lpips_loss_.mean().item(), style_loss_.mean().item()))
                            ...
                            utils.save_image(mask[0:4,:,:,:], f"mask_AOTGAN.png", nrow=2, normalize=False)
                            utils.save_image(input_save, f"input_AOTGAN.png", nrow=2, normalize=False)
                            utils.save_image(fake_img_save, f"out_AOTGAN.png", nrow=2, normalize=False)
